...
# direction. This adds bandwidth_gbps_h2d (mean of the bandwidth_gbps rows
# at the max transfer_size_bytes for direction=H2D) without disturbing the
# existing blind-mean "bandwidth_gbps"/"latency_us" summary values.
...
def add_peak_direction_bandwidth(rows, metrics_summary):
    ...
    Only H2D is exposed as a metric today (bandwidth_gbps_h2d); D2H/D2D are
    computed the same way here for consistency and future use, but are not
    currently cited in BenchmarkSpecDefinitions.xlsx.
    """
    ...
    for direction, direction_rows in by_direction.items():
        ...
        if peak_values:
            ...
            metrics_summary[f"bandwidth_gbps_{slug}"] = statistics.mean(peak_values)
