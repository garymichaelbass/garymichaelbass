...
# Description: Compares GPU PyTorch matmul/conv2d against a CPU FP32 reference for each yaml op/dtype/shape.
...
Compares GPU PyTorch matmul/conv2d against a CPU FP32 reference for each yaml op/dtype/shape.
...
Workload options:
  ...
  --op-name <value>            Override op_name
  --dtype <value>            Override dtype
  --shape <value>            Override shape
  --rtol <value>            Override rtol
  --atol <value>            Override atol
  --num-iterations <value>            Override num_iterations
  --seed <value>            Override seed
...
OP_NAME="${OP_NAME:-$(yaml_get op_name)}"
DTYPE="${DTYPE:-$(yaml_get dtype)}"
SHAPE="${SHAPE:-$(yaml_get shape)}"
RTOL="${RTOL:-$(yaml_get rtol)}"
ATOL="${ATOL:-$(yaml_get atol)}"
...
[[ -n "${OP_NAME}" ]] && BENCHMARK_RUN_COMMAND+=" --op-name ${OP_NAME}"
[[ -n "${DTYPE}" ]] && BENCHMARK_RUN_COMMAND+=" --dtype ${DTYPE}"
[[ -n "${SHAPE}" ]] && BENCHMARK_RUN_COMMAND+=" --shape ${SHAPE}"
[[ -n "${RTOL}" ]] && BENCHMARK_RUN_COMMAND+=" --rtol ${RTOL}"
[[ -n "${ATOL}" ]] && BENCHMARK_RUN_COMMAND+=" --atol ${ATOL}"
[[ -n "${NUM_ITERATIONS}" ]] && BENCHMARK_RUN_COMMAND+=" --num-iterations ${NUM_ITERATIONS}"
[[ -n "${SEED}" ]] && BENCHMARK_RUN_COMMAND+=" --seed ${SEED}"
...
run_collection() {
  ...
  grep -q -- '--op-name' <<<"${help}" && collect_args+=(--op-name "${OP_NAME}")
  grep -q -- '--dtype' <<<"${help}" && collect_args+=(--dtype "${DTYPE}")
  grep -q -- '--shape' <<<"${help}" && collect_args+=(--shape "${SHAPE}")
  grep -q -- '--rtol' <<<"${help}" && collect_args+=(--rtol "${RTOL}")
  grep -q -- '--atol' <<<"${help}" && collect_args+=(--atol "${ATOL}")
  grep -q -- '--num-iterations' <<<"${help}" && collect_args+=(--num-iterations "${NUM_ITERATIONS}")
  grep -q -- '--seed' <<<"${help}" && collect_args+=(--seed "${SEED}")
